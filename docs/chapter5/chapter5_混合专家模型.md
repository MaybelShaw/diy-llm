...
**概念直观理解：**
`MoE模型`就像一个拥有大量书籍的图书馆（专家集合）,当一个读者（输入数据）来访时，他并不需要翻遍所有书架，而是根据自己感兴趣的主题由图书管理员（路由机制）指引，只去相关书架寻找书籍区域（稀疏激活）。这样一来，读者仍然可以访问图书馆中所有书籍的知识（模型的巨大参数容量），同时查找过程却快速高效（FLOPs）。
...
**结论** TC在语义完整性上占优但易遭遇专家能力两极化；EC在负载均衡上占优但要承受潜在可能的语义丢失风险，两种模式代表了稀疏专家系统在信息完整性 vs 负载均衡上的典型权衡抉择，根据近期的研究也有解决这个权衡问题的思路：
| 策略名 | 核心思路 | 说明 |
| ------ | -------- | ----------- |
...
与传统卷积核的固定分工不同，**混合专家**模型引入了**条件路由**例如**Top-k 门控机制**，在前向计算时**只动态激活少数专家模块**。每个专家可以专注处理特定类型的输入或特征模式，从而在**显著提升模型容量**的同时，保持**FLOPs可控**。换句话说，MoE的分工是**输入驱动、动态且稀疏的**，而传统卷积核的分工是算子并行且固定的。
...
**在微调阶段对大模型引入RL**。以`Kimi-K2`为例（约1.04T参数，激活参数约32.6B），**研究团队结合混合并行与LoRA分片**，实现了稳定的RL训练，相比全参数RL该方法将GPU成本降低至约10%。对比实验表明：大基座模型+小规模LoRA的RL明显优于小模型的全参数RL，这一现象可归因于：`本阶段RL的效果受限于基座模型的先验能力`，强大的base model能产生更高质量的训练轨迹，从而显著提升RL的学习效率。超大规模`MoE`会遇到一些特殊挑战，例如：
